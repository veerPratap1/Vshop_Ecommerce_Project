import { useSelector, useDispatch } from "react-redux";
import {
  fetchBrandAsync,
  fetchCatagoryAsync,
  fetchSubCatagoryAsync,
  selectBrand,
  selectCatagory,
  selectProductStatus,
  selectSubCatagory,
  selectTotalItems,
} from "../ProductSlice";
import { useEffect, useState } from "react";

import { Link } from "react-router-dom";
import ReactLoading from "react-loading";
import { Carousel } from "@material-tailwind/react";
import { ITEMS_PAGE } from "../../../app/constant";

export default function HomePage(props) {
  const dispatch = useDispatch();

  const totalItems = useSelector(selectTotalItems);
  const catagories = useSelector(selectCatagory);
  const brand = useSelector(selectBrand);
  const subCategory = useSelector(selectSubCatagory);

  let [page, setPage] = useState(1);

  useEffect(() => {
    setPage(1);
  }, [totalItems]);

  useEffect(() => {
    const limit = { _limit: 4 };
    window.scrollTo(0, 0);

    dispatch(fetchCatagoryAsync(limit));
    dispatch(fetchBrandAsync(limit));
    dispatch(fetchSubCatagoryAsync(limit));
  }, [dispatch]);

  useEffect(() => {
    window.scrollTo(0, 0);
    const value = props.value;

    const newLabel = { value: value, _page: page, _limit: ITEMS_PAGE };

    dispatch(fetchSubCatagoryAsync(newLabel));
    dispatch(fetchCatagoryAsync(newLabel));
    dispatch(fetchBrandAsync(newLabel));

    if (!value) {
      const limit = { _limit: 4 };
      window.scrollTo(0, 0);

      dispatch(fetchCatagoryAsync(limit));
      dispatch(fetchBrandAsync(limit));
      dispatch(fetchSubCatagoryAsync(limit));
    }
  }, [dispatch, page, props.value]);

  return (
    <>
      <div className="bg-white dark:bg-gray-500  w-full">
        <div>
          {/* Mobile filter dialog */}

          <main className="">
            <section aria-labelledby="products-heading border" className="">
              <h2 id="products-heading" className="sr-only border">
                Products
              </h2>
              <div className="grid grid-cols-1 lg:grid-cols-1">
                {/* Filters */}
                {/* <DesktopFilter
                  handleFilter={handleFilter}
                  filters={filters}
                ></DesktopFilter> */}
                {/* Product grid */}
                <div className="lg:col-span-3 py-5">
                  {/* Your content */}
                  <ProductGrid
                    categories={catagories}
                    brand={brand}
                    subCategory={subCategory}
                  ></ProductGrid>
                </div>
              </div>
            </section>
          </main>
        </div>
      </div>
    </>
  );
}

function ProductGrid({ products, categories, subCategory, brand }) {
  const status = useSelector(selectProductStatus);
  return (
    <>
      <div className="bg-white dark:bg-gray-500">
        <div className="mx-auto max-w-2xl px-4 py-0 sm:px-6 sm:py-0 lg:max-w-7xl lg:px-8">
          <Carousel
            className="rounded-xl z-30 w-100 mt-8 h-48 sm:h-48 md:h-60 lg:h-96"
            transition={{ duration: 1 }}
          >
            <div className="relative h-full w-full">
              <img
                src="https://www.aptronixindia.com/media/rokanthemes/blog/images/5/g/5g_desktop_banner_try_1.png"
                alt="banner 1"
                className="h-full w-full object-fit"
              />
              <div className="absolute inset-0 flex items-end pb-10 justify-center h-48 sm:h-48 md:h-60 lg:h-96  w-full  ">
                <div className="w-3/4 md:w-2/4 flex justify-center">
                  <div className="flex gap-2">
                    <Link
                      to={"/shop"}
                      className="middle none center mt-3 rounded-full bg-pink-500  md:py-2 md:px-3 py-1 px-1 font-sans text-[8px] md:text-xs font-bold uppercase text-white shadow-md shadow-pink-500/20 transition-all hover:shadow-lg hover:shadow-pink-500/40 focus:opacity-[0.85] focus:shadow-none   disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                    >
                      Shop Now
                    </Link>
                  </div>
                </div>
              </div>
            </div>
            <div className="relative h-full w-full">
              <img
                src="https://m.media-amazon.com/images/S/stores-image-uploads-eu-prod/4/AmazonStores/A21TJRUUN4KGV/f2be8ea1629ab1bfedc95a486b637453.w1349.h434.jpg"
                alt="banner 2"
                className="h-full w-full object-fit"
              />
              <div className="absolute inset-0 flex items-end pb-10 justify-center h-48 sm:h-48 md:h-60 lg:h-96  w-full   ">
                <div className="w-3/4 md:w-2/4 flex justify-center">
                  <div className="flex gap-2">
                    <Link
                      to={"/shop"}
                      className="middle none center mt-3 rounded-full bg-pink-500 py-2 px-3 font-sans text-xs font-bold uppercase text-white shadow-md shadow-pink-500/20 transition-all hover:shadow-lg hover:shadow-pink-500/40 focus:opacity-[0.85] focus:shadow-none   disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                    >
                      Shop Now
                    </Link>
                  </div>
                </div>
              </div>
            </div>
            <div className="relative h-full w-full">
              <img
                src="https://i.pinimg.com/originals/d0/78/70/d078705c172a131d88c67bd19986172d.jpg"
                alt="banner 3"
                className="h-full w-full object-fit"
              />
              <div className="absolute inset-0 flex items-end pb-10 justify-center h-48 sm:h-48 md:h-60 lg:h-96  w-full    ">
                <div className="w-3/4 md:w-2/4 flex justify-center">
                  <div className="flex gap-2">
                    <Link
                      to={"/shop"}
                      className="middle none center mt-3 rounded-full bg-pink-500 py-2 px-3 font-sans text-xs font-bold uppercase text-white shadow-md shadow-pink-500/20 transition-all hover:shadow-lg hover:shadow-pink-500/40 focus:opacity-[0.85] focus:shadow-none   disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                    >
                      Shop Now
                    </Link>
                  </div>
                </div>
              </div>
            </div>
          </Carousel>
          {/* Category setion */}
          <section className="pt-10 mt-20 rounded-2xl bg-slate-100 px-3 ">
            <div className="container mx-auto">
              <div className="-mx-4 flex flex-wrap">
                <div className="w-full px-4">
                  <div className="mx-auto mb-12 max-w-[510px] text-center lg:mb-12">
                    <h2 className="mb-4 text-3xl font-bold text-dark sm:text-4xl md:text-[40px]">
                      Shop By Category
                    </h2>
                  </div>
                </div>
              </div>
              <div className=" flex flex-wrap">
                <div className="w-full px-4 grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-4 lg:grid-cols-4 xl:gap-x-8">
                  {status === "loading" ? (
                    <div className="grid min-h-full place-items-center px-6 py-24 sm:py-32 lg:px-8">
                      <ReactLoading
                        type={"spinningBubbles"}
                        color={"#a4bbc9"}
                        height={100}
                        width={100}
                      />
                    </div>
                  ) : (
                    categories?.slice(0,4).map((cat) => (
                      <div
                        key={cat.id}
                        className="group relative shadow-2xl p-2 bg-white mb-8 rounded"
                      >
                        <Link to={`/category/${cat.id}`}>
                          <div className="h-24  md:h-40  lg:h-60 overflow-hidden rounded-md lg:aspect-none group-hover:opacity-75 lg:h-60">
                            {status === "loading" ? (
                              <ReactLoading
                                type={"spinningBubbles"}
                                color={"#a4bbc9"}
                                height={50}
                                width={50}
                              />
                            ) : (
                              <img
                                src={`/${cat.label}`}
                                alt="catergory Img"
                                className="h-full w-full object-fit lg:h-full lg:w-full"
                              />
                            )}
                          </div>
                          <div className="mt-2 flex justify-center">
                            <h3 className="text-lg font-bold text-gray-700">
                              <div>
                                <span
                                  aria-hidden="true"
                                  className="absolute inset-0"
                                />
                                {cat.value}
                              </div>
                            </h3>
                          </div>
                        </Link>
                      </div>
                    ))
                  )}
                </div>
              </div>
              <div className="mx-auto pb-8 pt-3 max-w-[510px] text-center lg:mb-12">
                <Link
                  to={"/allCategory"}
                  className="middle none center rounded-full bg-blue-500 py-2 px-3 font-sans text-xs font-bold uppercase text-white shadow-md shadow-blue-500/20 transition-all hover:shadow-lg hover:shadow-blue-500/40 focus:opacity-[0.85] focus:shadow-none   disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                >
                  See all Categories
                </Link>
              </div>
            </div>
          </section>
          {/* Sub-Category setion */}
          <section className="pt-10 mt-20 rounded-2xl bg-slate-100 px-3">
            <div className="container mx-auto">
              <div className="-mx-4 flex flex-wrap">
                <div className="w-full px-4">
                  <div className="mx-auto mb-12 max-w-[510px] text-center lg:mb-12">
                    <h2 className="mb-4 text-3xl font-bold text-dark sm:text-4xl md:text-[40px]">
                      Shop By Sub-Category
                    </h2>
                  </div>
                </div>
              </div>
              <div className=" flex flex-wrap">
                <div className="w-full px-4 grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-4 lg:grid-cols-4 xl:gap-x-8">
                  {status === "loading" ? (
                    <div className=" py-24 sm:py-32 lg:px-8">
                      <ReactLoading
                        type={"spinningBubbles"}
                        color={"#a4bbc9"}
                        height={100}
                        width={100}
                      />
                    </div>
                  ) : (
                    subCategory?.slice(0,4).map((Subcat) => (
                      <div
                        key={Subcat.id}
                        className="group relative shadow-2xl p-2 bg-white mb-8 rounded"
                      >
                        <Link to={`/subCategory/${Subcat.id}`}>
                          <div className="h-24  md:h-40  lg:h-60 overflow-hidden rounded-md lg:aspect-none group-hover:opacity-75 lg:h-60">
                            {status === "loading" ? (
                              <ReactLoading
                                type={"spinningBubbles"}
                                color={"#a4bbc9"}
                                height={50}
                                width={50}
                              />
                            ) : (
                              <img
                                src={`/${Subcat.label}`}
                                alt="catergory Img"
                                className="h-full w-full object-fit lg:h-full lg:w-full"
                              />
                            )}
                          </div>
                          <div className="mt-2 flex justify-center">
                            <h3 className="text-lg font-bold text-gray-700">
                              <div>
                                <span
                                  aria-hidden="true"
                                  className="absolute inset-0"
                                />
                                {Subcat.value}
                              </div>
                            </h3>
                          </div>
                        </Link>
                      </div>
                    ))
                  )}
                </div>
              </div>
              <div className="mx-auto pb-8 pt-3 max-w-[510px] text-center lg:mb-12">
                <Link
                  to={"/allSubCategory"}
                  className="middle none center rounded-full bg-blue-500 py-2 px-3 font-sans text-xs font-bold uppercase text-white shadow-md shadow-blue-500/20 transition-all hover:shadow-lg hover:shadow-blue-500/40 focus:opacity-[0.85] focus:shadow-none   disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                >
                  See all Sub-Categories
                </Link>
              </div>
            </div>
          </section>
          {/* Brand setion */}
          <section className="pt-10 mt-20 rounded-2xl bg-slate-100 px-3">
            <div className="container mx-auto">
              <div className="-mx-4 flex flex-wrap">
                <div className="w-full px-4">
                  <div className="mx-auto mb-12 max-w-[510px] text-center lg:mb-12">
                    <h2 className="mb-4 text-3xl font-bold text-dark sm:text-4xl md:text-[40px]">
                      Shop By Brand
                    </h2>
                  </div>
                </div>
              </div>
              <div className=" flex flex-wrap">
                <div className="w-full px-4 grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-4 lg:grid-cols-4 xl:gap-x-8">
                  {status === "loading" ? (
                    <div className=" py-24 sm:py-32 lg:px-8">
                      <ReactLoading
                        type={"spinningBubbles"}
                        color={"#a4bbc9"}
                        height={100}
                        width={100}
                      />
                    </div>
                  ) : (
                    brand?.slice(0,4).map((brand) => (
                      <div
                        key={brand.id}
                        className="group relative shadow-2xl p-2 bg-white mb-8 rounded"
                      >
                        <Link to={"/brand/" + brand.id}>
                          <div className="h-24 md:h-40 lg:h-60 overflow-hidden rounded-md lg:aspect-none group-hover:opacity-75">
                            {status === "loading" ? (
                              <ReactLoading
                                type={"spinningBubbles"}
                                color={"#a4bbc9"}
                                height={50}
                                width={50}
                              />
                            ) : (
                              <img
                                src={`/${brand.label}`}
                                alt="catergory Img"
                                className="h-full w-full object-fit lg:h-full lg:w-full"
                              />
                            )}
                          </div>
                          <div className="mt-2 flex justify-center">
                            <h3 className="text-lg font-bold text-gray-700">
                              <div>
                                <span
                                  aria-hidden="true"
                                  className="absolute inset-0"
                                />
                                {brand.value}
                              </div>
                            </h3>
                          </div>
                        </Link>
                      </div>
                    ))
                  )}
                </div>
              </div>
              <div className="mx-auto pb-8 pt-3 max-w-[510px] text-center lg:mb-12">
                <Link
                  to={"/allBrand"}
                  className="middle none center rounded-full bg-blue-500 py-2 px-3 font-sans text-xs font-bold uppercase text-white shadow-md shadow-blue-500/20 transition-all hover:shadow-lg hover:shadow-blue-500/40 focus:opacity-[0.85] focus:shadow-none   disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                >
                  See all Brands
                </Link>
              </div>
            </div>
          </section>
        </div>
      </div>
    </>
  );
}
