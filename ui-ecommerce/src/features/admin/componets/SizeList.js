...
const SizeList = () => {
  ...
  const onSubmit = (data) => {
    ...
    dispatch(updateSizeAsync(item));
    if (status === "idle") {
      alert.success("Size is Successfully updated");
    }
    reset();
  };
  ...
  const serachInput = (e) => {
    ...
    dispatch(fetchSizeAsync(newValue));
  };
                          ...
                          <tr role="row" key={index}>
                            ...
                            <td className="py-3 text-sm" role="cell">
                              <div className="flex font-bold justify-center">
                                <Modal
                                  title={`${size.value}`}
                                  message="Do You Want To Delete This Size From List ?"
                                  cancelOption="Cancel"
                                  dangerOption="Remove"
                                  dangerAction={(e) => handleDelete(size.id)}
                                  cancelAction={(e) => handleCancel()}
                                  openModal={openModal === size.id}
                                ></Modal>
                                <button
